...
(function() {
  ...
  if (window.location.protocol === 'https:') {
    ...
    function startPlugin() {
        ...
        Lampa.Manifest.plugins = Lampa.Manifest.plugins || {}; Lampa.Manifest.plugins.showy = manifst;
        Lampa.Lang.add({
            lampac_watch: { //
...
// === SHOWY ICON BUTTON (FULL FIX) ===
(function () {
    function addShowyButton(ctx) {
        if (!ctx || !ctx.render || !ctx.render.length) return;
        if (ctx.render.find('.showy-icon-btn').length) return;

        var btn = $(
            '<div class="full-start__button selector showy-icon-btn">' +
        ...
        btn.on('hover:enter', function () {
            ...
            Lampa.Activity.push({
                ...
                title: Lampa.Lang.translate('title_online'),
                movie: ctx.movie,
                search: ctx.movie.title,
                search_one: ctx.movie.title,
                search_two: ctx.movie.original_title,
                page: 1
            });
        });

        ctx.render.append(btn);
    }
    ...
    Lampa.Listener.follow('full_start', function (e) {
        try {
            ...
            console.log('Showy button error', err);
        }
    });
})();
...
playback timings (ms):
  ...
  esindex: 0.011
  cdx.remote: 46.834
  LoadShardBlock: 331.84 (3)
  PetaboxLoader3.datanode: 551.029 (5)
  load_resource: 264.479
